...
func (e *Encoder) EncodeLength(l uint32) (err error) {
	...
	case l < 1<<14:
		_, err = e.w.Write([]byte{byte(l>>8) | structure.RDB14BitLen<<6, byte(l)})
	default:
		b := make([]byte, 5)
		b[0] = structure.RDB32or64BitLen << 6
...
func (e *Encoder) encodeIntString(b []byte) (written bool, err error) {
	...
	case i >= math.MinInt8 && i <= math.MaxInt8:
		_, err = e.w.Write([]byte{structure.RDBEncVal << 6, byte(int8(i))})
	case i >= math.MinInt16 && i <= math.MaxInt16:
		b := make([]byte, 3)
		b[0] = structure.RDBEncVal<<6 | structure.RDBEncInt16
		binary.LittleEndian.PutUint16(b[1:], uint16(int16(i)))
		_, err = e.w.Write(b)
	case i >= math.MinInt32 && i <= math.MaxInt32:
		b := make([]byte, 5)
		b[0] = structure.RDBEncVal<<6 | structure.RDBEncInt32
